...
//+------------------------------------------------------------------+
#property copyright   "布林带马丁 M1 v5.0"
#property version     "5.00"
#property description "纯距离驱动马丁 (XAU M1) v5.0 - 入场诊断版"
#property strict
...
//+------------------------------------------------------------------+
//| L1开仓 (v5.0 诊断版: 每根新K线输出本次入场判断的全部细节)             |
//+------------------------------------------------------------------+
void CheckEntry()
{
   ...
   if(!IsTradeTime())
   {
      Print("[诊断] ", tag, "× 非交易时段 (", Inp_StartHour, "-", Inp_EndHour, "h)");
      return;
   }
...
void UpdatePanel()
{
   ...
   CreateLbl(panelName+"t", 10, y, "=== BB马丁 M1 v5.0 ===", clrGold); y += lh + 4;
